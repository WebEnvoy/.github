...
- Viewer 必须展示明确选择的原 Instance／任务页，不冒称该页是 OS 前台；原生坐标、键盘和其他输入必须可靠落到该目标窗口和画面，否则拒绝输入。
...
- 原生选中／前台关系未知但 Page 归属、新鲜度和只读路径仍可验证时，可以继续读取或诊断；不能确认输入落点、授权或身份时，只拒绝相关动作，必要时暂停该上下文，不扩大为无关资源的全局失败。
...
- 上传必须确认页面实际接收并处理文件；
- 发布和保存必须防止重复触发；
- 原生坐标、键盘和其他输入必须有可靠的目标 Page／窗口关系；无法确认实际落点时拒绝输入。
...
- Viewer 的明确选中页不等于 OS 前台页；只读查看不得主动激活，原生输入落点不可靠时必须拒绝。
...
无法确认结果时不得自动重试。任务页引用失效、输入落点或身份无法确认时，也必须拒绝相关写入并保留可验证的恢复入口。
...
39. `closePage` 只关闭获准目标；目标或最后可用 Page 无法确认时拒绝，不借邻页、URL、标题或创建顺序认领，不隐式结束 Instance 或创建替代页；原生输入落点无法确认时拒绝输入。
...
> **任务页引用与原生选中／前台焦点分离。任务页 A 仍可信时，人类在同一 Instance 中观看或操作其他页 B 后交还，Agent 默认重新观察 A；只读不激活，Viewer 不冒称 OS 前台，输入落点或 Page 归属不明时拒绝。A 失联时只能依据供应方可信关系或用户明确选择且仍可验证的真实 Page 恢复，不得按 URL、标题、内容或创建顺序认领，不得复制、重载、重建或重放。**
...
| 任务页与原生选中／前台焦点分离；只读不激活，Viewer 不冒称 OS 前台，输入落点不明即拒绝 | 已确认原则 |
