...
> **2026-09-17 可选模型辅助修订：** 允许用户明确启用有界的模型辅助网页任务，保留原有 Agent／人类使用路径；Model Provider 与 Browser Provider 分开，模型判断不代替权限或外部结果。该方向暂不增加 V1 发布门槛，具体模型的收益与采用范围仍待验证，详见 §12.7。
...
本文既有 ProviderExecutable、ProviderBinding、Profile 迁移及第 9 章中的 Provider 均指 **Browser Provider（浏览器供应方）**。第 12.7 节新增的 **Model Provider（模型接入服务）** 是独立选择维度，不拥有 Profile，不继承浏览器绑定或迁移语义；模型 evaluation 也不是受控网页脚本执行。
...
- 通用自主 Browser Agent（不排除 §12.7 中明确目标、授权范围与停止条件内的可选模型辅助）；
- 自建大模型对话产品；
...
每个 Provider／Driver 必须按版本和平台分别声明 capability 的 `supported`、`limited` 或 `unsupported`。未测试不等于 `unsupported`；支持状态、证据来源、运行可用性和授权状态分别表达。不同 Provider 不要求具备完全相同的能力等级；某候选受限不能删除 V1 Runtime 能力类别，也不能反向要求 WebEnvoy 补其核心缺口。声明的组合必须是供应方正式发布、来源可核对且未经 WebEnvoy 改写的组合。
...
WebEnvoy 可以在用户明确启用和授权的范围内，使用模型辅助完成网页任务。用户可以委托一段有明确目标、网页范围、已有操作和停止条件的连续任务，不必每一步都回到通用 Agent 决策；也可以完全不使用该能力。它不改变第 1 章定位，不建设通用聊天、开放式业务规划或通用自主 Agent 平台。

### 模型与用途分开

| 概念 | 含义 |
|---|---|
| Model Provider | 用户批准使用的模型接入服务，包括直连、Gateway 或未来本地服务；不是 Browser Provider。 |
| Model | 具体请求模型及供应方实际返回的可识别版本。 |
| Model Capability | 模型实际具备且有证据支持的能力与限制，例如有界选择、文本或视觉。 |
| Model Usage | WebEnvoy 使用模型的目的、允许数据、选择范围、停止和结果规则。 |

模型连接可以被已获准用途复用，不按决策／文本／视觉建立多套 Provider。语义分开不要求现在新建独立服务、存储或完整模型平台。模型选择不绑定长期 Profile，也不改变 Browser Provider、设备环境或账号归属；一次辅助段内固定请求模型与批准的路由边界，全局默认变化不能造成在途静默替换。别名不等于固定权重，实际版本未提供时必须如实未知。

### 选择、执行和结果的边界

模型只在当前可信观察提供的有限、已支持候选中选择操作和目标。每次实际派发仍由现有 Core／Harbor 重新核对权限、Page／document、目标身份与状态、ControlLease 和执行条件；模型输出、置信度和工具可见性都不是授权。

模型可以选择 DONE／BLOCKED。DONE 只结束它认为不必继续的辅助循环，业务结果仍按任务实际具备的独立证据核验；没有核验方法就保留未确认，不能假设存在万能 Verifier。BLOCKED 表示该辅助路径不能继续，不自动等于整个任务失败。无需为了形式让另一模型再说一次“完成”。

通用 Agent 可以承担复杂理解、计划或必要文本生成，也可以只委托一段任务并接收结果。模型不拥有 Profile、正式账号归属、权限、浏览器现场或结果真相；Harbor 不替用户决定业务下一步。首个正式消费优先通过已安装 Plugin，Plugin 保持薄层；不为本方向新增完整聊天入口或第二套 Run／恢复系统。

### 用户选择、数据与控制

默认关闭。保存模型密钥、测试连接或设置默认不自动允许真实网页外发或执行；连接测试使用合成内容。网页访问权、模型使用权、数据向实际接收方外发的同意及费用范围必须分别说明，并由现有可信 owner／授权路径承载。密钥不交普通 Agent、Plugin 日志或页面。

只发送本次用途必要、获准且脱敏的有界信息，默认不发送截图；密码、验证码、Cookie、token、原始 Profile、私有端点和无关或未获准内容不得随模型请求外发。Gateway 的实际接收方、路由和留存边界也必须可解释。页面内容始终是不可信数据，不能改写目标或授权。

停止、撤权、连接失效或人工接管按原规则阻止新派发；迟到的模型答复不得复活循环。明确交还后重新观察原任务页。关闭辅助不删除长期环境、历史结果，也不自动关闭原浏览器。模型请求重试、SDK 恢复或模型替换都不得重放已派发且结果未知的网页写入。

退出辅助可以将已做动作、原 Run／receipt、未确认结果和安全下一步交回实际发起任务的宿主。宿主缺席或用户直接发起时进入待处理，不暗中启动另一模型，也不从第一步重做。另一模型或推理服务的自动路由须处于用户明确批准的用途与外发范围，不得静默替换。

### 采用与版本归属

**[待原型验证]** Jev 作为首个候选的速度、总成本、可靠性与可维护性，应在同一 WebEnvoy 基线和结果标准下验证；“更快、更便宜、更准确”是收益目标，不是本规范已经证实的能力。上游模型目录或 SDK 支持不自动等于 WebEnvoy 产品支持。

**[V1 收敛约束]** 本方向暂不增加第 22 章的 V1 发布门槛。产品归口为 [#558](https://github.com/WebEnvoy/WebEnvoy/issues/558)，首个条件验证为 [#559](https://github.com/WebEnvoy/WebEnvoy/issues/559)；它们属于 M25 条件性后续。条件齐备可以提前验证，不要求整个 V1 先结束；提前交付也不豁免权限、结果、停止、恢复和安装验证。

SDK 采用、发布包资格及临时适配的进入／退出条件由仓内 ADR 与行为规格承接，不在本规范锁定第三方 API 或承诺其支持日期。现有 #555／#556 继续独立，不等待模型接入，也不因本方向扩大原完成标准。
...
- Viewer 的明确选中页不等于 OS 前台页；只读查看不得主动激活。依赖 OS 窗口、屏幕坐标或原生键盘路由的输入若无法证明窗口与画面对应则拒绝，Page／Element 对象级输入按 Page／document、target/actionability、授权和 ControlLease 核验。
...
36. 真实 Provider 验收使用来源可核对、未经 WebEnvoy 改写的供应方正式浏览器／驱动组合；WebEnvoy 仅保留正式配置／协议调用、Driver 转换、权限和环境 bundle 管理。
...
41. 旧私有 patch binding、补丁合同和复制改写 bundle 从当前安装／启动／Runtime 路径退役，但旧绑定、数据、授权、账号、Run／receipt、SKILL 和恢复事实仍可查询、管理、撤销、对账和安全恢复。
...
| 通用 Browser Agent | 明确非目标；§12.7 的可选有界辅助不属于通用自主平台 |
| 通用 Model Provider 与 Browser Provider 分离，可选辅助不改变原使用路径 | 已确认原则 |
| 模型可结束有界循环，业务结果仍须独立核验；模型不授予权限 | 已确认原则 |
| Jev 的效率、成本、可靠性与采用范围 | 待原型验证 |
| 可选模型辅助暂不增加 V1 发布门槛，条件齐备可提前验证 | V1 收敛约束 |
