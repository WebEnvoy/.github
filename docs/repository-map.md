...
| `WebEnvoy/App` | monorepo `App` 模块 | 验收前旧仓保持权威；迁移必须保留 App Shell 和独立进程边界。 |
| `WebEnvoy/Harbor` | monorepo `Harbor` 模块 | 验收前旧仓保持权威；迁移不得带入 Profile、credential、Cookie、token 或未脱敏运行现场。 |
| `WebEnvoy/Lode` | 独立 `WebEnvoy/Lode` | 不迁入 monorepo、不使用 submodule；只通过 version/hash pin 和 compatibility CI 消费。 |

目标拓扑是路线和迁移目标，不表示当前已经完成源码导入、workspace、CI、Desktop 装配、入口切换或仓库退役。

## 跨模块职责与数据流

以下关系中的 `Core`、`App`、`Harbor` 是模块名称；迁移完成前，它们分别由上表的独立仓库承载。
...
迁移只在下列门全部通过、证据绑定同一迁移 head 并由 owner 明确确认后，才允许切换 GitHub 执行入口。任一门失败或事实不可用，都停止切换并继续以现有多仓为权威。
...
通过迁移门后仍需单独完成可回滚发布和 GitHub mapping 的 owner 确认；本文件和本批次 PR 不执行源码迁移、入口切换、transfer、close 或 archive。

## 失败、暂停与回滚

- 合同、历史、clean-checkout、运行时装配、数据/许可证、GitHub mapping 或回滚门任一未通过时，保留现有多仓和旧入口；不把文档状态写成迁移完成。
- GitHub、仓库、许可证或关键架构事实暂时不可读时暂停更新，不用推测补齐缺口，并记录 `needs-decision` 或待补证据。
- 文档 PR 自身校验失败时，只回滚本次四份文档改动，恢复原地图、许可证和治理入口；不改产品代码或仓库设置。
- 迁移切换后的任何回读、发布或运行时验证失败，都恢复旧 App/Harbor 入口和权威分支，保留差异和证据；旧仓在明确验收前不归档。

## 延后条件（Deferred）

- 源码导入、单一 workspace、按路径 CI、Desktop 装配和运行时行为变更，延后到合同、历史、clean-checkout 和运行时装配证据齐备的后续 Work Items。
- GitHub 执行真相切换、组织入口切换以及 App/Harbor 的 transfer、close 或 archive，延后到 GitHub mapping、可回滚发布、readback、rollback evidence 和 owner 明确确认全部完成之后。
